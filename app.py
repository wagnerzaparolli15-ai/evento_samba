import os, mercadopago, datetime
...
app.secret_key = os.getenv("SECRET_KEY", "SISTEMA_BAFAFA_2026_MASTER")

db = SQLAlchemy(app)
sdk = mercadopago.SDK(os.getenv("MP_ACCESS_TOKEN"))

# --- MODELOS (A ESTRUTURA DO EVENTO) ---
...
class Cliente(db.Model):
    ...
    hora_entrada = db.Column(db.String(20), default="")
...
# --- O PAINEL DE CONTROLE NA PALMA DA MÃO ---
...
def admin_total():
    ...
    fin = {
        ...
        "total_vendas": ingressos_pago,
        "na_casa": Cliente.query.filter_by(na_casa=True).count()
    }
    
    return render_template('admin_total.html', equipe=Equipe.query.all(), produtos=prod_all, clientes=Cliente.query.filter_by(na_casa=True).all(), fin=fin)
